...
<div style="max-width:640px">
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:24px">
        ...
        <h2 style="font-size:20px;font-weight:600;color:#111827;margin:0">Nuevo Proyecto</h2>
    </div>
    ...
    <div style="background:#fff;border-radius:12px;box-shadow:0 1px 4px rgba(0,0,0,.08);padding:28px">
        <form method="POST" action="{{ route('proyectos.store') }}">
            ...
            <div style="margin-bottom:16px">
                ...
                @error('id_cliente')<p style="color:#dc2626;font-size:12px;margin-top:4px">{{ $message }}</p>@enderror
            </div>

            <div style="margin-bottom:16px">
                <label style="display:block;font-size:13px;font-weight:500;color:#374151;margin-bottom:4px">Nombre del proyecto *</label>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:16px">
                <div>
                    ...
                    <input type="date" name="fecha_inicio" value="{{ old('fecha_inicio') }}" required
                           style="width:100%;border:1px solid #d1d5db;border-radius:8px;padding:8px 12px;font-size:14px;box-sizing:border-box">
                    @error('fecha_inicio')<p style="color:#dc2626;font-size:12px;margin-top:4px">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label style="display:block;font-size:13px;font-weight:500;color:#374151;margin-bottom:4px">Fecha fin estimada *</label>
                    <input type="date" name="fecha_fin_estimada" value="{{ old('fecha_fin_estimada') }}" required
                           style="width:100%;border:1px solid #d1d5db;border-radius:8px;padding:8px 12px;font-size:14px;box-sizing:border-box">
                    @error('fecha_fin_estimada')<p style="color:#dc2626;font-size:12px;margin-top:4px">{{ $message }}</p>@enderror
                </div>
            </div>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:24px">
                ...
                <div>
                    ...
                    <input type="number" name="presupuesto" value="{{ old('presupuesto') }}" step="0.01" min="0"
                           style="width:100%;border:1px solid #d1d5db;border-radius:8px;padding:8px 12px;font-size:14px;box-sizing:border-box">
                </div>
            </div>
